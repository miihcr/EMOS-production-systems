...
addresses_input <- addresses_raw |> 
  ...
  select(id_address, house_number, house_addition,
         postcode, id_public_space, start_valid, end_valid)
...
public_spaces_input <- public_spaces_raw |> 
  ...
  select(id_public_space, public_space_name, public_space_type,
...
dwellings_input <- dwellings_raw |> 
  ...
  select(id_dwelling, usage_purpose, area_m2, status_clean,
...
towns_input <- towns_raw |> 
  ...
  select(id_town, town_name, town_status,
...
buildings_input <- buildings_raw |> 
  ...
  select(id_building, construction_year, building_status,
         start_valid, end_valid, x_coord, y_coord)
...
sales_input <- sales_raw |> 
  ...
  select(full_address, street_name, house_number, house_addition,
